...
const CustomTooltip: React.FC<CustomTooltipProps> = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      ...
      const value = typeof data.Value === 'number' ? Math.exp(data.Value.toFixed(2)) : 'N/A';
...
const DisplayStreamsTimeline: React.FC<DisplayStreamsDataProps> = ({ rawData }) => {
  ...
  return (
    <div style={{ width: '100%', height: '500px', marginTop: '20rem' }}>
      ...
      <ComposedChart data={chartData} margin={{ top: 200, right: 50, left: 50, bottom: 50, }}>
        ...
        <Line type="monotone" dataKey="Value" name="Timeline" stroke="#82ca9d" strokeWidth={4}/>
